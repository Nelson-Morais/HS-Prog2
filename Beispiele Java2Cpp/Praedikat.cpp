...
public:
 ...
 bool operator()(int zahl) const {      // prueft, ob zahl Vielfaches
...
vector<int> find_if(const vector<int>& vec, const IstVielfachesVon& vielfach_n) {
 ...
 for (size_t i=0; i<vec.size(); i++) {
  if (vielfach_n(vec[i])) ergebnis.push_back(vec[i]);
    //         ^-- Aufruf von IstVielfachesVon::operator()(int)
 }
 return ergebnis;
}
